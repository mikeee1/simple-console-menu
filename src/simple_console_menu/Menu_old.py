    ...
    while chooseLoop:
        ...
        if onlyReturnNumber:
            ...
            if acceptedQuitCharacters != "":
                if (
                    (choose != "" and choose in allowedCharactersList)
                    or choose.isdigit()
                    or choose == acceptedQuitCharacters
                ):
                    chooseLoop = False
...
def SimpleConsoleMenuBlock(
    ...
    if (menuNameLength % 2) == 0:
        ...
        menuNameLength2 = int(math.floor(menuNameLength))
        print(
            "╭" + int(menuNameLength1 - 1) * "─",
            menuName,
            int(menuNameLength2 - 1) * "─" + "╮",
        )
    for x in menuItemsList:
        menuItemsWithNumbers = str(menuNumber) + ". " + x
        print(
            f"│{menuItemsWithNumbers}"
            + ((menuSize - 2) - len(menuItemsWithNumbers)) * " "
            + "│"
        )
        menuNumber += 1
    print("╰" + (menuSize - 2) * "─" + "╯")
